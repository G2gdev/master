...
class Restaurant_model extends CI_Model {
	...
	public function getSearchedVendor($map) {
		...
		$nowTime = '"'.date('H:i:s').'"';
		$distance = '6371000 * acos( cos( radians(' . $map ['latitude'] . ') ) * cos( radians( a.latitude ) ) * cos( radians( a.longitude ) - radians(' . $map ['longitude'] . ') ) + sin( radians(' . $map ['latitude'] . ') ) * sin( radians( a.latitude ) ) )';
		$this->db->select ( 'a.id,a.radius, a.name,a.garage_name,a.email, a.mobile, a.mobile, IFNULL( 6371000 * acos( cos( radians(' . $map ['latitude'] . ') ) * cos( radians( a.latitude ) ) * cos( radians( a.longitude ) - radians(' . $map ['longitude'] . ') ) + sin( radians(' . $map ['latitude'] . ') ) * sin( radians( a.latitude ) ) ) ,0) AS distance' )->from ( TABLES::$RESTAURANT . ' AS a' );
		//$this->db->where ( 'a.status', 1 );
		$this->db->having ( 'distance < a.radius', '', FALSE );
		$query = $this->db->get();
		$result = $query->result_array ();
