...
class Order_model extends CI_Model {
	...
	public function getOrderDetails($ordercode) {
		$this->db->select('a.*,b.name, b.mobile, b.email,b.outstanding,e.invoice_url,e.invoice_date,e.id as invoice_id,e.status as invoice_s,f.transactionid,f.status as payment_status,f.payment_date,
	...
	public function getPendingOrdersByDate() {
		...
		$this->db->select('a.*,b.name, b.mobile, b.email, c.garage_name');
		$this->db->from(TABLES::$ORDER.' AS a');
		$this->db->join(TABLES::$USER.' AS b','a.userid = b.id','inner');
		$this->db->join(TABLES::$RESTAURANT.' AS c','a.vendor_id = c.id','left');
		$this->db->where('a.status',0);
...
// 	}
	...
	public function getOrderItems($orderid) {
        ...
        public function getOrderItems2($orderid) {
		$this->db->select('a.*', FALSE)
				 ->from(TABLES::$ORDER_SERVICE.' AS a')
				// ->join(TABLES::$SERVICE.' AS b','a.service_id=b.id','inner')
				// ->join(TABLES::$SPARE.' AS c','a.service_id=c.id','inner')
...
// 	}	
	...
	public function getUserOrders($userinput) {
		$this->db->select('a.*,b.name, b.mobile, b.email,b.area,c.garage_name');
		$this->db->from(TABLES::$ORDER.' AS a');
		$this->db->join(TABLES::$USER.' AS b','a.userid = b.id','inner');
		$this->db->join(TABLES::$RESTAURANT.' AS c','a.vendor_id = c.id','left');
		$this->db->where('a.mobile',$userinput);		
		$this->db->order_by('a.orderid','desc');
		$query = $this->db->get();
		$result = $query->result_array();
		return $result;
