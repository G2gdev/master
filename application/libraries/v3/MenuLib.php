...
class MenuLib {
	...
	public function getRestaurants($map){
		$this->CI->load->model ( 'v3/restaurants/Menu_model', 'menu');
	...
	public function uploadSheet($template) {
		...
		$this->CI->load->model ( 'v3/restaurants/Menu_model', 'menu');
		$this->CI->load->model ( 'v3/restaurants/Restaurant_model','restaurant');
		$this->CI->load->model ( 'v3/restaurants/Items_model','items');
		...
		foreach($mcategories as $key=>$row)
		{
			if(isset($row['categories']) && count($row['categories']) > 0) {
				...
				foreach($categories as $key1=>$row1)
				{
					...
					foreach($items as $key3=>$row3)
					{
						$category = array('name'=>$row3['name'],'category_id'=>$row3['category_id'],'brand_id'=>$row3['brand_id'],'model_id'=>$row3['model_id'],'subcategory_id'=>$row3['subcategory_id'],'price'=>$row3['price'],'tax'=>$row3['tax'],'menu_uploaded'=>1);
		...
		foreach($mcategories as $key=>$row)
		{
			if(isset($row['categories']) && count($row['categories']) > 0) {
				...
				foreach($categories as $key1=>$row1)
				{
					...
					foreach($items as $key3=>$row3)
					{
						$category = array('name'=>$row3['name'],'category_id'=>$row3['category_id'],'brand_id'=>$row3['brand_id'],'model_id'=>$row3['model_id'],'subcategory_id'=>$row3['subcategory_id'],'catsubcat_id'=>$row3['catsubcat_id'],'price'=>$row3['price'],'tax'=>$row3['tax'],'menu_uploaded'=>1);
	...
	public function updateitems($items) {
		$this->CI->load->model ( 'v3/restaurants/Menu_model','menus');
		$item_batch = array();
		$price_batch = array();
		$restid = 0;
	...
	public function publishmenu($data) {
		...
		$batch = array();
		$this->CI->load->model ( 'v3/restaurants/items_model','items');
		$this->CI->load->model ( 'v3/restaurants/options_model','options');
