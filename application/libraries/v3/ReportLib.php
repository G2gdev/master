...
class ReportLib {
	...
	public function getBusinessReport($params) {
		...
		$reports = array();
		$garagecomm = 0; $vendorcomm=0;$grandtotal = 0;
		foreach ($orders as $order) {
			$report = array();
			...
			$grandtotal = $report['grand_total'] + $grandtotal;
			...
			if(!empty($order['grand_total'])){
				...
				$garagecomm = $garagecomm + $report['garagecomm'] ;

				$report['vendorcomm'] =	number_format($report['amount_received'] - $report['garagecomm'],2);
				$vendorcomm = $vendorcomm + $report['vendorcomm'] ; 
			}else{
				$report['garagecomm'] = 0;
				$report['vendorcomm'] = 0;
			}
			$items = $this->CI->order->getOrderItems2($order['orderid']);
		...
		$reports['total_garagecomm'] = $garagecomm;
		$reports['total_vendorcomm'] = $vendorcomm ;
		$reports['total_revenue'] = $grandtotal ; 
