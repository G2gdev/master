...
class UserLoginLib {
	...
	public function userExist($data) {
		$this->CI->load->model ( 'v3/users/UserLogin_model', 'userlogin' );
	...
	public function userRegistration($data) {
		...
		try {
			$user_id = $this->CI->userlogin->userRegistration ( $data );
				...
				$this->CI->load->model ( 'v3/users/Wallet_model', 'wallet' );
	...
	public function validateReferralCode($reffralcode){
		$this->CI->load->model ( 'v3/users/UserLogin_model', 'userloginmodel' );
	...
	public function sendSignUpSMS($details) {
		...
		$fname = $name[0];
		//$sms_msg = 'Hi '.$fname.',Use verification code ' . $details ['otp'] . ' to complete the registration process on Garage 2 Ghar.';
		$sms_msg = 'Hi '.$fname.', Thanks for registering with us! You can now access our Breakdown, Doorstep & Pickup & Drop services with complete ease using Garage2Ghar App OR Website. Simply book your appointment any time you wish and get updates on your order, offers and options to pay online. Thanks! Garage2Ghar Team';
		$this->CI->load->library ( 'Pksms' );
		$map = array ();
		$map ['mobile'] = $details ['mobile'];
		$map ['message'] = $sms_msg;
	...
	{
		//$this->CI->load->model ( 'v3/User', 'fblogin' );
