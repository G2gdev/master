...
class OrderLib {
	...
	public function sendBookingEmail($data) {
		...
		$this->CI->pkemail->headline = 'Booking Request |Garage2Ghar';
	...
	public function sendCancelEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Booking Request Cancelled | Garage2Ghar';
		$this->CI->pkemail->subject = 'Booking Request Cancelled | Garage2Ghar';
	...
	public function sendOrderConfirmEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Booking order confirm |Garage2Ghar';
	...
	public function sendRescheduleEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Reschedule order |Garage2Ghar';
	...
	public function sendGenrateEstimateEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Estimate Genrated | Garage2Ghar';
	...
	public function sendPickUpEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Order Ready For PickUp | Garage2Ghar';
		$this->CI->pkemail->subject = 'Order Ready For PickUp | Garage2Ghar';
	...
	public function sendPickedUpEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Your Order Got Tagged in your name | Garage2Ghar';
		$this->CI->pkemail->subject = 'Your Order Got Tagged in your name | Garage2Ghar';
	...
	public function sendDeliveryEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Your Order is ready to be delivered | Garage2Ghar';
		$this->CI->pkemail->subject = 'Your Order is ready to be delivered | Garage2Ghar';
	...
	public function sendInvoiceEmail($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Your Order Invoice | Garage2Ghar';
		$this->CI->pkemail->subject = 'Your Order Invoice | Garage2Ghar';
	...
	public function sendBookingSMS($data) {
		...
		$fname = $name[0];
       //$sms_msg = 'Hi '.$fname.', Garage2Ghar is happy to receive your appointment request. Our team will confirm the request shortly.';
        $sms_msg = 'Dear '.$fname.', Thank you for choosing Garage2Ghar. You will be notified once a mechanic is assigned for your service. For queries contact us on 9011941194';
	...
	public function sendCancelSMS($data) {
		$name = explode(" ",$data['name']);
		$fname = $name[0];
		//$sms_msg = 'Dear ' . $data ['name'] . ' , Your order has been cancelled. We would like to hear from you. Please get in touch with us. Regards, Garage2Ghar';
	...
	public function sendOrderConfirmSMS($data) {
		$name = explode(" ",$data['name']);
		$fname = $name[0];
		//$sms_msg = 'Hi '.$fname.', your appointment has been confirmed and we will be there to serve you at the given time and address. Thanks for choosing Garage2Ghar.';
	...
	public function sendInvoiceSMS($data) {
		...
		//$sms_msg = 'Hello ' . $data ['name'] . ' , Your bill amounts to Rs. '.$data['bill_amount'].'. Please Check your invoice. '.$data['payment_url'].' Regards, Garage2Ghar';
		//$sms_msg = 'Hello ' . $fname . ' , Thanks again for choosing Garage2Ghar. Kindly check Invoice & Pay '.$data['payment_url'].'.';
...
// 	}
	...
	//	$sms_msg = 'Hi ' . $data ['name'] . ' , Your order is ready to be delivered. The bill amounts to Rs '.$data['bill_amount'].'. The Moustache Man, '.$data['delivery_executive'].' ('.$data['delivery_executive_mobile'].') will be there at the schedule time, be there to say Hi! Regards, The Moustache Laundry';
		$sms_msg = 'Clean clothes ready! Garage2Ghar will call you shortly to confirm delivery today. Check Invoice & Pay: <URL>';
		$this->CI->load->library ( 'pksms' );
		$map = array ();
		$map ['mobile'] = $data ['mobile'];
		$map ['message'] = $sms_msg;
	...
	public function deliveryAttemptedSMS($data) {
		$sms_msg = 'Hi There, We were unsuccessful in our attempt to deliver your order. Please call us back. Regards, Garage2Ghar';
		$this->CI->load->library ( 'pksms' );
		$map = array ();
		$map ['mobile'] = $data ['mobile'];
		$map ['message'] = $sms_msg;
	...
	public function deliveryCallAnsweredSMS($data) {
		$sms_msg = 'Hi There, We tried reaching you but couldn\'t connect. Please let us know time for your delivery or call us back. Regards, Garage2Ghar';
		$this->CI->load->library ( 'pksms' );
		$map = array ();
		$map ['mobile'] = $data ['mobile'];
		$map ['message'] = $sms_msg;
	...
	public function pickupAttemptedSMS($data) {
		$sms_msg = 'Hi There, We were unsuccessful in our attempt to pick up your order. Please call us back. Regards, Garage2Ghar';
		$this->CI->load->library ( 'pksms' );
		$map = array ();
		$map ['mobile'] = $data ['mobile'];
		$map ['message'] = $sms_msg;
	...
	public function confirmApproval_sms($data) {
		$name = explode(" ",$data['name']);
		$fname = $name[0];
		//$sms_msg = 'Hi '.$fname.', Thanks for confirming the estimate. Have mailed you the details. Will inform you once we complete the services. Thanks, Garage2Ghar!';
	...
	public function confirmApproval_email($data) { 
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Estimate confirmation | Garage2Ghar';
	...
	public function markworkCompleted_sms($data) {
		$name = explode(" ",$data['name']);
		$fname = $name[0];
		//$sms_msg = 'Hi '.$fname.', Your Bike is ready ! Thanks, Team Garage2Ghar!';
	...
	/* public function markworkCompleted_email($data) {
		...
		$this->CI->load->library ( 'pkemail' );
		$messages ="<html>
		Hi ". $fname.",<br><br>Your Bike is ready! Thanks again for choosing Garage2Ghar.<br/><br /> 
				Thanks, <br />
				Team Garage2Ghar!
				</body>
				</html>";
	...
	public function markworkCompleted_email($data) {
		$this->CI->load->library ( 'pkemail' );
		$this->CI->pkemail->load_system_config ();
		$this->CI->pkemail->headline = 'Work Completed | Garage2Ghar';
	...
	public function sendGenrateEstimateSMS($data) {
		$name = explode(" ",$data['name']);
		$fname = $name[0];
		//$sms_msg = 'Hi '.$fname.', We have generated and mailed you an estimate based on our expert inspection. Kindly check and confirm the same to start the services. Thanks, Team Garage2Ghar!';
	...
	public function sendBookingNotification($userid) {
		...
		if(!empty($gcm_id)){
			...
			$msg = 'Appointment booked. Thank you for choosing Garage2Ghar. You will be notified once a mechanic is assigned for your service '.$curr_date.' ';
			$message = array
			(
					'body' => $msg,
					'title' => $title,
...
// 	}
	...
	foreach($datanew as $data){
		...
		<b>Garage Mobile Number :</b> ".$data['garage_name']." <br />
		<b>Garage Name :</b> ".$data['garage_name']." <br />
...
// 	}
	...
	public function getUserOrders($userinput) {
		$this->CI->load->model ('orders/Order_model','order');
		return $this->CI->order->getUserOrders($userinput);
	}
}