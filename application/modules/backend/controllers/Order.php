...
class Order extends MX_Controller {
	...
	public function rejectedOrders(){
		...
		if(empty($this->input->post('name')) && empty($this->input->post('delivery_date')) && empty($this->input->post('mobile')) && empty($this->input->post('email'))) {
			$orders = $this->orderlib->getRejectedOrdersByDate();
		} else {
			$map = array();
			$map['status'] = 1;
			//$map['role_id'] = $role_id;
		...
		->set_partial ( 'leftnav', 'partials/sidebar' )
		->set_partial ( 'footer', 'partials/footer' );
		$this->template->build ('orders/RejectedOrders');
	}
	...
	public function orderDetail($orderid) {
		...
		$data3=array();
		//category_id pass to data3 array
		$data3['category_id'] = $orders[0]['category_id'];
	...
	public function assignDelivery($orderid) {
		...
		$data['vehicle_no'] = $orders[0]['vehicle_no'];
		$data['category_name'] = $orders[0]['category_name'];
		$data['brand_name'] = $orders[0]['brand_name'];
		$data['model_name'] = $orders[0]['model_name'];
		$data['comment'] = $orders[0]['comment'];
		
		$this->orderlib->sendOrderConfirmEmail($data);
		$this->orderlib->sendOrderConfirmSMS($data);
		$this->orderlib->sendAssignedgarageNotification($orders[0]['userid'],$orderid);
		$this->mechaniclib->sendGarageAssignedNotification($garage_id);
                 
		//$this->mechaniclib->sendGarageAssignedEmail($garage_id,$data);
	...
	public function updateassigndelivery($orderid) {
		...
		//$orderdata['tml_delivery_date'] = date('Y-m-d',strtotime($this->input->get('delivery_date')));
		$orderdata['vendor_response'] =1;
		$orderdata['is_accepted'] =0;
		$orderdata['accepted_by'] =0;
	...
	public function validateInput($key,$params)
	{
		$error = array();
		foreach ($params[$key] as $key => $value) {
			if(!is_numeric($value)){
				$error['status'] = 0;
				$error['msg'] = 'Number Field Required';
				return $error;
			}
			...
			{
				$error['status'] = 0;
				$error['msg'] = 'Quantity should be greator than zero.';
				return $error;
			}
			...
			{
				$error['status'] = 0;
				$error['msg'] = 'decimal or float value will not be accepted.';
				return $error;
			}
		}
		return $error;
	...
	public function addItems() {
        ...
        $validation = $this->validateInput("qty",$this->input->post());
        if(!empty($validation)){
		...
		for ($i = 0; $i < count($itemids); $i++) {
			...
			$item['quantity'] = $itempqty[$i];
			$item['priority'] = $itempriority[$i];
			$price = $itemprices[$i] * $itempqty[$i];
			$cal=$itemtaxes[$i] /100 * $price;
			$item['total_amount']= round($price + $cal);
			$items[] = $item;
			$ordertotal = round($ordertotal + $item['total_amount']);
		}
		...
		$orderdata['net_total'] = round($ordertotal - $discount + $delivery_charge);
		$orderdata['grand_total'] = round($ordertotal - $discount + $delivery_charge + $adjustment);
		$orderdata['status'] = 2;
		...
		$this->orderlib->sendEstimategeneratedNotification($orders[0]['userid'],$orderid);
	...
	public function updateItems() {
        ...
        $validation = $this->validateInput("itemqty",$this->input->post());
        if(!empty($validation)){
        	$response['status'] = $validation['status'];
            $response['message'] = $validation['msg'];
            echo json_encode($response);
		...
		$itempriority = $this->input->post('priority');
		$itempqty = $this->input->post('itemqty');
		...
		for ($i = 0; $i < count($itemids); $i++) {
			...
			if($itemids[$i]==''){
				$response['status'] = 0;
				$response['message'] = 'Please enter valid service';
                echo json_encode($response);
                exit;
			}
			$item['service_id'] = $itemids[$i];
	...
	public function completeOrder($orderid) {
		...
		$this->orderlib->sendDeliveryCompleteNotification($orders[0]['userid'],$orderid);
    ...
    public function updateServices() {
		...
		$itemtypes = $this->input->post('itemnm'); 
 		$itempqty = $this->input->post('qtyA');
		$itempriority = $this->input->post('itempriority');
		$itemnm = $this->input->post('itemnm');  
    	$qtys = $this->input->post('qty');
    	$wts = $this->input->post('weight');
    	$validation = $this->validateInput("qtyA",$this->input->post());
        if(!empty($validation)){
        	$response['status'] = 1;
    		$response['message'] = $validation['msg'];
    		echo json_encode($response); 
    	...
    	for ($i = 0; $i < count($itemids); $i++) {
    		...
    		if($item['is_checked'] == 1){
    			$total_amount = round($price + $cal);
    		}else{
    			$total_amount = 0;
    		}
    		$items[] = $item;
   			$ordertotal = round($ordertotal + $total_amount); 
		}
    	$this->orderlib->removeOrderItems($orderid);
    	$this->orderlib->addOrderItems($items);
    	$items = $this->orderlib->getOrderItems($orderid);
    	...
    	$orderdata['adjustment'] = $adjustment;
    	$orderdata['net_total'] = round($ordertotal - $discount + $delivery_charge);
    	$orderdata['grand_total'] = round($ordertotal - $discount + $adjustment + $delivery_charge);
    	$this->orderlib->updateOrder($orderdata);
    ...
    public function getReminderuserdata() {
    	...
    	$todaysTickets = $this->orderlib->getTodaysTickets();
    	$this->template->set('page','porders');
    	$this->template->set('ordersremind',$ordersremind);
    	$this->template->set('todaysTickets',$todaysTickets);
    	$this->template->set_theme('default_theme');
    	$this->template->set_layout ('backend')
    	->title ( 'Administrator | Order Dashboard' )
    	->set_partial ( 'header', 'partials/header' )
    ...
    public function pdfdemo() {
            ...
            $file_name = "invoice_".rand(0,100).".pdf";
            $this->load->library('MyPdfLib');
            $url = $this->mypdflib->getPdf($html,$file_name);
            echo base_url().$url;
    }
    ...
    public function addReminder(){
    	$this->load->library('zyk/OrderLib');
    	$reminder = array();
    	$reminder['orderid'] = $this->input->post('orderid');
    	$reminder['reminder_date'] = date('Y-m-d',strtotime($this->input->post('reminder_date')));
    	//$reminder['reminder_time'] = $this->input->post('reminder_time');
    	$reminder['reminder_comment'] = $this->input->post('reminder_comment');
    	$reminder['current_date'] =  date('Y-m-d H:i:s');
    	$id = $this->orderlib->addReminder($reminder);
    	$response = array();
    	if(!empty($id)) {
    		$response['status'] = 1;
    		...
    		$response['status'] = 0;
    		$response['message'] = "Failed to set Reminder.";
    	}
    	echo json_encode($response);
    
    }
    public function addremindercron(){
    	$this->load->library('zyk/OrderLib');
    	$reminder = $this->orderlib->getRemindercron();
//     	echo '<pre>';
//     	print_r($reminder);
//     	exit;
    	//$admin = $this->orderlib->getAdminDetails();
    //	$this->orderlib->sendReminderSMS($reminder,$admin);
    	$this->orderlib->sendReminderEmail($reminder);
    }
    
    
 
    public function showReminderList() {
    	$current_date = date('Y-m-d');
    	$this->load->library('zyk/OrderLib');
    	$reminder = $this->orderlib->getReminderList();   	
    	...
    	->set_partial ( 'leftnav', 'partials/sidebar' )
    	->set_partial ( 'footer', 'partials/footer' );
    	$this->template->build ('orders/ReminderList');
    }

    public function order_list(){
		$id = $this->input->post('userid');
		$this->load->library('zyk/UserLib','userlib');
		$order_list = $this->userlib->getOrder_five($id);
 		$this->template->set('orders',$order_list); 
 		$this->template->set_theme('default_theme');
		$this->template->set_layout (false);
 		$this->template->build ('general/order_list');
	}
    ...
    public function send_mail() {
		...
		$this->pkemail->headline = 'Booking Request |Garage2Ghar';
		$this->pkemail->subject = 'We have received your order.';
		$this->pkemail->mctag = 'booking';
		$this->pkemail->attachment = 0;
		$this->pkemail->to = 'tabrosh@brandzgarage.com';
		$this->template->set_layout ( false );
		$text_body = $this->template->build ( 'frontend/home', '', true );
    ...
    public function userOrders() {
		...
		$this->load->library('zyk/OrderLib');
		$adminsession = $this->session->userdata ( 'adminsession' );
		$userinput = $this->input->get('userinput');

		if(!empty($userinput)){
			$orders = $this->orderlib->getUserOrders($userinput);
			$this->template->set('page','porders');
			$this->template->set('orders',$orders);
			$this->template->set_theme('default_theme');
			$this->template->set_layout ('backend')
			...
			->set_partial ( 'leftnav', 'partials/sidebar' )
			->set_partial ( 'footer', 'partials/footer' );
			$this->template->build ('orders/UserOrders');
	    }
	}
