					...
					<div class="col-lg-12">
						<div class="panel panel-default">
							...
							<div class="row"> 
								...
								<div class="row">
									...
									<div class="col-lg-6 margin-bottom-5">
										<div class="form-group" id="error-subject">
											...
											<div class="col-sm-10"> 
												<input type="text" class="form-control" name="ticket[subject]" id="other" 
												placeholder="subject" />
											</div>
											<div class="messageContainer col-sm-10"></div>
										</div>
								...
								<div class="row">
									<div id="order_list" class="col-lg-8"></div>
								</div>
							</div>
						</div>
					</div>
...
$('#addticket').bootstrapValidator({
    ...
    fields: {
        ...
        'ticket[category_id]': {
            validators: {
                notEmpty: {
                    message: 'Please select sub category from dropdown'
                }
            }
        },
        'ticket[subcategory_id]': {
            validators: {
                notEmpty: {
                    message: 'Please select order from dropdown'
                }
            }
        },
        'ticket[subject]': {
        ...
        'ticket[priority]': {
            validators: {
                notEmpty: {
                    message: 'Please select priority from dropdown'
                }
            }
        },
        'ticket[assigned_to]': {
        ...
        'ticket[status]': {
            validators: {
                notEmpty: {
                    message: 'Please select status from dropdown'
                }
            }
        },
    }
...
$("#mobile").typeahead({
    onSelect: function(item) {
        ...
        $.get(base_url+"admin/user/detail/"+item.value,{},function(result){
			...
			$('#order_list').empty(); 
	  		$.post(base_url+"admin/order_list", {userid : result.id }, function(data) {
		 		  $('#order_list').append(data);	   
      		},'html');

      		 $.post(base_url+"admin/details/", {userid : result.id}, function(data) {
