...
class Api extends REST_Controller {
	...
	public function login_post() {
		...
		{
		   $this->load->library ( 'v3/UserLoginLib' );
	...
	public function checkaddress_post() {
	  ...
	  public function addUseraddress_post() {
		...
		public function contact_post() {
			...
			$this->load->library('v3/Adminauth');
		...
		public function getOngoingOrder_post() {  
			$data = array();
			$data['user_id'] = $this->post('user_id');
		 
			if($this->post('user_id')!="") {
				$this->load->library ( 'v3/userLib' );
				$data = $this->userlib->getOngoingOrders_byuser($data['user_id']); 
				if(!empty($data)){	
					$resp['success']='true';
					$resp["msg"] = "Order List Successfully";
					$resp['data']=$data;
				}else{
					$resp['success']='false';
